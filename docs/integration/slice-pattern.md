...
This memo extracts the **shared anatomy** of the first two vertical slices that proved the constitutional runtime against live domains, and names the first Reconcile-shaped slice that deliberately uses a different anatomy. It is descriptive, not prescriptive — it names what already worked so future slices can recognize the pattern instead of rediscovering it.
...
- **`install.reconcile`** (reconcile-anatomy) — state-convergence action. Proves the runtime can plan drift, apply bounded sub-steps, make partial convergence explicit, and close by correlation without treating provider/sub-step success as constitutional closure.

All reconstruct cleanly from the ledger by `correlation_id`, under shared simulation and failure semantics.
...
## 5. What the single-act pattern excludes
...
- **Reconcile-shaped operations** that converge desired and applied state over multiple sub-executes. `install.reconcile` is now the first landed example; see `docs/integration/reconcile-anatomy.md`.
