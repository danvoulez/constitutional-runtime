...
impl DispatchOutcome {
    pub fn into_outbound_send(self) -> Result<OutboundSendOutcome, StoreError> {
        match self {
            ...
            Self::HostPair(_) | Self::InstallReconcile(_) => Err(StoreError::Contract(
    ...
    pub fn into_host_pair(self) -> Result<HostPairOutcome, StoreError> {
        match self {
            ...
            Self::OutboundSend(_) | Self::InstallReconcile(_) => Err(StoreError::Contract(
    ...
    pub fn into_install_reconcile(self) -> Result<InstallReconcileOutcome, StoreError> {
        match self {
            ...
            Self::OutboundSend(_) | Self::HostPair(_) => Err(StoreError::Contract(
                "dispatcher returned non-install outcome for install.reconcile caller".into(),
            )),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn lower_execute_action_builds_platform_target_for_install_reconcile() {
        let mut params = BTreeMap::new();
        params.insert("installation_id".into(), json!(Uuid::nil()));
        params.insert("correlation_id".into(), json!(Uuid::nil()));

        let (_, command) =
            lower_execute_action("dispatch-install", "install.reconcile", params).unwrap();
        assert_eq!(command.namespace, "install");
        assert_eq!(command.verb, "reconcile");
