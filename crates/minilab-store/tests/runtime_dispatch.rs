...
async fn seed_install_reads(server: &MockServer, seed: &InstallSeed) {
    Mock::given(method("GET"))
        .and(path("/rest/v1/installation"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!([{
            "host_id": seed.host_id,
            "desired_manifest": { "services": { "api": "v2" } },
            "observed_manifest": { "services": { "api": "v1" } },
            "canon_version": "canon-v1",
            "elastic_version": "elastic-v1",
        }])))
        .mount(server)
...
#[tokio::test]
async fn lowered_install_reconcile_command_dispatches_into_the_reconcile_slice() {
    let server = MockServer::start().await;
    let seed = InstallSeed::fresh();
    seed_install_reads(&server, &seed).await;

    let client = client_for(&server, SimMode::Simulation);
    let correlation_id = Uuid::new_v4();
    let mut params = BTreeMap::new();
    params.insert("installation_id".into(), json!(seed.installation_id));
    params.insert("correlation_id".into(), json!(correlation_id));

    let (_, command) = lower_execute_action("dispatch-install", "install.reconcile", params)
        .expect("install.reconcile must lower");
    assert_eq!(command.target_runtime, RuntimeTarget::Platform);

    let outcome = dispatch_operational_command(&client, command)
        .await
        .expect("dispatcher must route install.reconcile");

    assert!(
        matches!(
            outcome,
            DispatchOutcome::InstallReconcile(
                minilab_store::InstallReconcileOutcome::Reconciled { .. }
            )
        ),
        "dispatcher must land in the install.reconcile slice"
    );
